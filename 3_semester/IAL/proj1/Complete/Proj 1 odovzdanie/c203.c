...
*/
		...
		{
			q->arr[i] = '*';	// Vsetky prvky nastavime na *
		}
		q->f_index = 0;	// Nastavime index prvej hodnoty v poli na 0
		q->b_index = 0;	// Nastavime index volneho miestna na 0
}
...
*/
	return (index + 1) % QUEUE_SIZE;	// Vratime index nasledujuceho prvku vo fronte
}
...
*/
	/*
		Fronta je prazdna iba vtedy ked ukazovatel na zaciatok zoznamu je ten
		isty ako ukazovatel na dalsie prazdne miesto
	*/
...
*/
		return (q->f_index == nextIndex(q->b_index)) ? 1 : 0;	// Ak nasledujuce volne miesto bude ukazovat na prvy prvok fronty tak sme zaplnili celu frontu
}
...
*/
		if (queueEmpty(q) == 1)	// Ak je fronta prazdna
		{
			queueError(QERR_FRONT);	// Vratime error
			return;
		}

		*c = q->arr[q->f_index];	// Vratime prvy charakter z fronty
}
...
*/
		if (queueEmpty(q) == 1)	// Ak je fronta prazdna
		{
			queueError(QERR_REMOVE);	// Vratime error
			return;
		}

		q->f_index = nextIndex(q->f_index); // Posunieme ukazovatel na prvy prvok na nasledujuci prvok
}
...
*/
		if (queueEmpty(q) == 1)	// AK je fronta prazdna
		{
			queueError(QERR_GET);	// Vratime error
			return;
		}
		queueFront(q,c);	// Vratime prvy charakter z fronty
		queueRemove(q);		// Vymazeme prvy charakter z fronty
}
...
*/
	if (queueFull(q) == 1)	// Ak je fronta plna
	{
		queueError(QERR_UP);	// Vratime error
		return;
	}

	q->arr[q->b_index] = c;							// Vlozime charakter na nasledujuce prazdne miesto vo fronte
	q->b_index = nextIndex(q->b_index);	// Posunieme ukazovatel na nasledujuce prazdne miesto
}
