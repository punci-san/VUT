...
**/
	...
	if ((*RootPtr) != NULL)
	{
		...
		// Ak sme nasli ten spravny NODE
		else if ((*RootPtr)->Key == K)
		{
			// Treba zistit o aky typ nodu sa jedna

			// AK je to prazdny node ten staci len smazat
			...
			// Inac ak mame len lavy node
			else if ((*RootPtr)->LPtr != NULL && (*RootPtr)->RPtr == NULL)
			{
				// Tak si vlozime tento node do temporaly variable
				tBSTNodePtr tmp = (*RootPtr);
				// Prepiseme pointer na tento node aby ukazoval na lavy node
				(*RootPtr) = (*RootPtr)->LPtr;
				// A uvolnime node
				free(tmp);
			}
			// Inac ak mame len pravy node
			else if ((*RootPtr)->LPtr == NULL && (*RootPtr)->RPtr != NULL)
			{
				// Tak si vlozime tento node do temporaly variable
				tBSTNodePtr tmp = (*RootPtr);
				// Prepiseme pointer na tento node aby ukazoval na pravy node
				(*RootPtr) = (*RootPtr)->RPtr;
				// A uvolnime node
				free(tmp);
			}
			// Inac ak Node obsahuje oba Nody
			else if ((*RootPtr)->LPtr != NULL && (*RootPtr)->RPtr != NULL)
			{
				// Musime pouzit pomocnu funkciu ReplaceByRightmost
...
**/
		...
		*/
		...
		if (*RootPtr != NULL)
		{
			// Ak sa nieco nachadza v lavom node
			if ((*RootPtr)->LPtr != NULL)
			{
				// Mazeme najprv to
				BSTDispose(&(*RootPtr)->LPtr);
			}
			// Inac ak sa nieco nachadza v pravom node
