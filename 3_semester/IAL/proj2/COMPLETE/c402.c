...
**/
	...
	// Vytvorime item na pridanie
	tBTNodePtr item = malloc(sizeof(struct tBTNode));
	item->LPtr = NULL;
	item->RPtr = NULL;
	item->Cont = Content;

	// Zacneme prechadzat a hladat
	while (curr != NULL)
	{
		// Content je mensi nez tento node ideme vlavo
		if (Content < curr->Cont)
		{
			// AK sme nasli ten spravny Pointer na pridanie
			if (curr->LPtr == NULL)
			{
				// Tak pridame a ukoncime
				curr->LPtr = item;
				return;
			}
			// Inac ideme vlavo
		...
		// Content je vacsi nez tento node ideme vpravo
		else if (curr->Cont < Content)
		{
			// Ak sme nasli ten spravny pointer na pravo na pridanie
			if (curr->RPtr == NULL)
			{
				// Pridame item
				curr->RPtr = item;
		...
		// Inac sme nasli nieco co uz mame vtedy nerobime nic
		else
		{
			// Uvolnime item
			free(item);
	...
	// Ak je curr null znamena to ze pridavame prvy item tak ho pridame
	*RootPtr = item;
}
